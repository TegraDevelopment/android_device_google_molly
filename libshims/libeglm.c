 ...
 * [Dmitry Grinberg] Problems:
  ...
  * A tricky but clever solution: INTERPOSITION library:
      1. We'll edit the libEGL_tegra_impl.so and replace one of its "NEED" record with one referencing a new library
         which we'll create. Need a library name? why not "libeglm.so"? We'll also edit the function name
         from dlopen to LibLdr() to make sure our function gets called.
      2. Make sure that libeglm.so's NEED records include the library whose record we replaced in libnvos.so
         library, to make sure that the linker brings it in afterall and all symbols in it are found
      3. Implement libeglm.so such that it provides LibLdr() and backs that wil a proper call to dlopen()
         with the proper path

  * Result: GFX libraries works on M, with the help of libeglm.so.
 ...
 * [npjohnson]:
     ...
     # 1) NvOsLibraryLoad -> molly_m_libload
     sed -i 's/NvOsLibraryLoad/molly_m_libload/g' libEGL_tegra_impl.so

     # 2) libnvos.so -> libeglm.so
     sed -i 's/libnvos\.so/libeglm.so/g' libEGL_tegra_impl.so

     Wire this up in extract-files.sh, we'll move to blob_fixup
     in later LineageOS versions which support it.
 */

#define LOG_TAG "libeglm.so"
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <utils/Log.h>
